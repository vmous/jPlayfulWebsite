...
import providers.PlayfulUsernamePasswordAuthProvider.PlayfulIdentity;
import providers.PlayfulUsernamePasswordAuthUser;
...
public class SignUp extends Controller {
	...
	private static final Form<PlayfulIdentity> FORGOT_PASSWORD_FORM = form(PlayfulIdentity.class);

	public static Result forgotPassword(final String email) {
		com.feth.play.module.pa.controllers.Authenticate.noCache(response());
		Form<PlayfulIdentity> form = FORGOT_PASSWORD_FORM;
		if (email != null && !email.trim().isEmpty()) {
			form = FORGOT_PASSWORD_FORM.fill(new PlayfulIdentity(email));
		}
	...
	public static Result doForgotPassword() {
		com.feth.play.module.pa.controllers.Authenticate.noCache(response());
		final Form<PlayfulIdentity> filledForm = FORGOT_PASSWORD_FORM
				.bindFromRequest();
		if (filledForm.hasErrors()) {
			...
			if (user != null) {
				...
				// reset, though.
				final PlayfulUsernamePasswordAuthProvider provider = PlayfulUsernamePasswordAuthProvider
						.getProvider();
				// User exists
	...
	public static Result doResetPassword() {
		...
		if (filledForm.hasErrors()) {
			...
			try {
				...
				// happen
				u.resetPassword(new PlayfulUsernamePasswordAuthUser(newPassword),
						false);
			} catch (final RuntimeException re) {
				flash(Application.FLASH_MESSAGE_KEY,
						Messages.get("playauthenticate.reset_password.message.no_password_account"));
			}
			final boolean login = PlayfulUsernamePasswordAuthProvider.getProvider()
			...
			if (login) {
				...
				return PlayAuthenticate.loginAndRedirect(ctx(),
						new PlayfulLoginUsernamePasswordAuthUser(u.email));
			} else {
	...
	public static Result verify(final String token) {
		...
		if (Application.getCurrentUser(session()) != null) {
			return redirect(routes.Application.index());
		} else {
			return redirect(routes.Application.signIn());
		}
